...
class Search<T> {
  ...
  search(List<T> list,dynamic keyword,{String? operatorT}){
    if (T==Medicine && keyword.runtimeType.toString() == "double" && operatorT==">") {
    ...
    }else if(T==Medicine && keyword.runtimeType.toString() == "String" && operatorT=="==") {
      list.forEach((element ) {
        if ((element as Medicine).category==keyword) {
          (element as Medicine).display();
        }
      });
    }else if(T==Medicine && keyword.runtimeType.toString() == "String" && keyword.length==1&& operatorT==null) {
      list.forEach((element ) {
        if ((element as Medicine).name.toString().contains(keyword)) {
          (element as Medicine).display();
        }
      });}
    if(T == Client && keyword.runtimeType.toString() == "String" && operatorT.runtimeType.toString() == "String"){
    ...
    if (T == Medicine && keyword.runtimeType.toString()== "int") {
      List<Medicine> li=[];
      list.forEach((element) {
