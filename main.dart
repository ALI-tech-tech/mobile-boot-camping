...
void main(List<String> args) {
  Search searchMedicine=Search<Medicine>();
  Search searchClient=Search<Client>();
  ...
  List<Medicine> medicines=List.generate(5, (index) => Medicine(name: "Medicine ${index}", unitprice: Random().nextInt(100000), cat: "Category ${index}", expireDate: "${Random().nextInt(30)}/${Random().nextInt(12)}/${Random().nextInt(2023)}", sup: sups[index]));
  Invoice iv=Invoice();
...
iv.invoDetail.forEach((element) {
  ...
  print("--------------------->4000-------------------");
 searchMedicine.search(medicines, 4000.0 ,operatorT: ">");
 print("---------------------category 4-------------------");
 searchMedicine.search(medicines, "Category 4",operatorT: "==");
  print("---------------------Search for Medicines that contains 4 in their name-------------------");
searchMedicine.search(medicines, "4") ;
 print("---------------------Search for invoices for client 1 and with 1Medicine 1 name-------------------");
searchClient.search(clints, "Client 1",operatorT: "Medicine 1");
 print("---------------------Get the first 3 medicines with expire date after 2000-------------------");
searchMedicine.search(medicines, 2000);
